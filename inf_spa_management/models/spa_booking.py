...
class SpaBooking(models.Model):
    ...
    def get_calendar_data(self, branch, beautician, service):
        # if self.env.user.has_group('hr.group_hr_user') or self.env.user.has_group('hr.group_hr_manager'):
            print(branch,beautician,service)
    ...
    def action_save(self):
        self.ensure_one()
        if self.env.context.get('calendar_booking', False):
            return {
...
class SpaBookingLine(models.Model):
    ...
    service_request_ids_domain = fields.Binary(compute="_compute_request_domain", readonly=False)
    ...
    def _compute_request_domain(self):
        for rec in self:
            ...
            emp_domain = [
                ('is_beautician', '=', True),
                ('product_ids.id', '=?', rec.product_id.product_tmpl_id.id),
                ('branch_id', '=', rec.booking_id.branch_id.id),
            ]
            beauticians = self.env['hr.employee'].sudo().search(emp_domain)
            avail_beauticians = beauticians.filtered(lambda r: r.check_availablity(rec.from_date, rec.to_date,rec._origin.id,rec.booking_id.name,rec.booking_id.overlap_timing))
            if rec.employee_id not in avail_beauticians:
                rec.employee_id = False
            domain = [('id', 'in', avail_beauticians.ids)]
            rec.service_request_ids_domain = domain

    def name_get(self):
        res = super(SpaBookingLine, self).name_get()
        if self.env.context.get('show_beautician_name', False):
            res = []
            for line in self:
