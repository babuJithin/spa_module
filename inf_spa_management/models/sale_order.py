...
class SaleOrder(models.Model):
    ...
    def action_save_and_close(self):
        self.ensure_one()
        if self.env.context.get('calendar_booking'):
            return {
    ...
    def action_confirm_booking(self):
        ...
        if not booking:
            booking = self.env['spa.booking'].browse(self.env.context.get('booking_id'))
        ...
        action = self.env.ref('inf_spa_management.spa_payment_action').sudo().read()[0]
        action['context'] = {
