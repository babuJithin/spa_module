odoo.define('inf_spa_management.calendar_booking', function (require) {
    "use strict";
    ...
    var BookingCalendar = AbstractAction.extend({
        ...
        get_calendar_data: function (event, noupdate) {
            if (noupdate) {
                return;
            }
            var self = this;
            ...
            console.log('Service selection:', service, service_tmpl_id);
            self.$el.empty();
            ...
            rpc.query({
                ...
                // Reset selectors and trigger native change events
                $('#branch_selector').val(branch).trigger('change');
                $('#employee_selector').val(employee).trigger('change');
                $('#service_selector').val(service).trigger('change');

                var scroll_time = new Date(Date.now() - (30 * 60 * 1000));
                ...
                self.$el.find('#calendar').fullCalendar({
                    ...
                    views: {
                        day: { titleFormat: 'dddd, MMMM Do - YYYY' },
                    },
                ...
                self.$el.find('#calendar').on('swipe', function () {
                    $(this).hide();
                });

                if (gotoDate) {
                    self.$el.find('#calendar').fullCalendar('gotoDate', gotoDate);
                }
            });
        },
    });
